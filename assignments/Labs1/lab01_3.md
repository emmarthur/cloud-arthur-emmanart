...
![Marketplace UI with filters and WordPress card](images/section2_marketplace_ui.png)
...
Go to Compute Engine on the Google Cloud Platform console. Create 3 VM instances with a range of boot disks from the `pdx-cs` GCP project. To do so, when creating the instance, within the operating system selection UI, click on "Change".

**Operating system and storage**

Then navigate to "Custom images", change the project to the `pdx-cs` project, and select an image from the dropdown menu.

![Section 2: Complete content including Option #2](images/section2_complete.png)
...
![Section 3: Scan target for service instructions](images/section3_nmap_scan.png)
...
Compute Engine automatically creates per-zone default subnetworks and will place any VM instantiated into them. However, based on what the project requires, custom networks and subnetworks can also be specified. Using the gcloud CLI, we will now set up a network consisting of custom subnetworks.

![Section 4: Complete CIDR and Subnets content](images/section4_complete.png)
